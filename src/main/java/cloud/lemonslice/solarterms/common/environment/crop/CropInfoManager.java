...
public final class CropInfoManager
{
    ...
    public static void init(TagsUpdatedEvent event)
    {
        ...
        Arrays.asList(CropHumidityType.values()).forEach(type ->
                ItemTags.getAllTags().getTagOrEmpty(type.getRes()).getValues().forEach(crop -> registerCropHumidityInfo(crop, type)));
        Arrays.asList(CropSeasonType.values()).forEach(type ->
                ItemTags.getAllTags().getTagOrEmpty(type.getRes()).getValues().forEach(crop -> registerCropSeasonInfo(crop, type)));
