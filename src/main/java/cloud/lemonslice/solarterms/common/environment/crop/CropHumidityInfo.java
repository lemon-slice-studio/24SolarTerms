...
public class CropHumidityInfo
{
    ...
    public List<Component> getTooltip()
    {
        ...
        list.add(new TranslatableComponent("info.solarterms.environment.humidity").withStyle(ChatFormatting.GRAY));
        if (min != max)
        {
            list.add(((TranslatableComponent) min.getTranslation()).append(new TextComponent(" - ").withStyle(ChatFormatting.GRAY)).append(max.getTranslation()));
        }
        else
        {
